// Solution to part 2 of https://adventofcode.com/2023/day/16
// Mark Brudnak
// Dec 28, 2023
...
struct Contraption
{
    ...
    void reset()
    {
        ...
        {
            visitCounts[r][c] = 0;
            visitedFromLeft[r][c] = false;
            visitedFromRight[r][c] = false;
            visitedFromTop[r][c] = false;
            visitedFromBottom[r][c] = false;
        }
    }
...
int main( int argc, char *argv[] )
{
    ...
    {
        contraption.reset();
        contraption.doBeam( r, 0, 0, 1 );
        maxCount = max( maxCount, contraption.getVisitCount() );

        contraption.reset();
        contraption.doBeam( r, C-1, 0, -1 );
        maxCount = max( maxCount, contraption.getVisitCount() );
    }
    ...
    {
        contraption.reset();
        contraption.doBeam( 0, c, 1, 0 );
        maxCount = max( maxCount, contraption.getVisitCount() );

        contraption.reset();
        contraption.doBeam( R-1, c, -1, 0 );
        maxCount = max( maxCount, contraption.getVisitCount() );
    }
